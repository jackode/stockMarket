...
dweather = sp.genfromtxt('../CSV Data/dbDump/dweather_jorhat.csv', delimiter=';')
ddweather = sp.genfromtxt('../CSV Data/dbDump/ddweather_jorhat.csv', delimiter=';')
features35 = sp.genfromtxt('../CSV Data/dbDump/features_tyroon.csv', delimiter=';')
dfeatures35 = sp.genfromtxt('../CSV Data/dbDump/dfeatures_tyroon.csv', delimiter=';')
...
pi.dump(dayFeatures, file)

#Read dayFeatures from file:
# file = open('p.pickle', 'rb')
# pi.load(file)

#NEXT: Divide it into  sets. training: 70%, cv(for rfe nbFeaturesToSelect): 20%, test(real score): 10%
...
for i in xrange(testLen):
    testIds.append(randint(0, featLen-1))

for i in xrange(cvLen):
    cvIds.append(randint(0, featLen-1))

print 'Saving CV- and Test- sets'
pi.dump(cvIds, file)
